...
async function searchLocation() {
    ...
    try {
        ...
        if (data.length > 0) {
            ...
            // 🎯 核心更動：丟給 Render 後端直接去撈這個地點周遭的所有車位！
            ...
            if (listEl) listEl.innerHTML = `<div class="text-center py-20 text-red-500 font-bold">在台北市找不到「${query}」<br><span class="text-xs text-slate-400">請輸入更具體的名字</span></div>`;
        }
    } catch (err) {
...
function createSearchMarker(name, lat, lng, address = "") {
    ...
    // 🌟 核心加值：綁定常駐文字標籤 (Tooltip)，達成綠色文字標籤的效果
    ...
    if (searchInput && autocompleteList) {
        searchInput.addEventListener('input', function() {
            ...
            debounceTimer = setTimeout(async () => {
                try {
                    // 🎯 核心改進 1：移除強制前綴，並加上 addressdetails=1 獲取詳細地址
                    ...
                    if (suggestions.length > 0) {
                        ...
                        searchAllDiv.innerHTML = `<div class="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-bold flex-shrink-0">🔍</div><div class="text-sm text-slate-800 font-bold flex-1">搜尋「${query}」周邊車位</div>`;
                        ...
                        suggestions.forEach(place => {
                            ...
                            // 如果沒抓到詳細路段，就退回使用系統給的完整字串，並扣除店名避免重複
                        ...
                        // 🎯 核心改進 3：如果真的找不到，給予友善提示而不是一片空白
                        autocompleteList.innerHTML = `<div class="p-4 text-center text-sm text-slate-400 font-bold">找不到相關地點 😢<br><span class="text-xs font-normal">請嘗試輸入更完整的名稱或地址</span></div>`;
                        autocompleteList.classList.remove('hidden');
                    }
