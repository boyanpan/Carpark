...
function createSearchMarker(name, lat, lng, address = "") {
    ...
    // 🌟 核心加值：綁定常駐文字標籤 (Tooltip)，達成圖六綠色文字標籤的效果
    ...
    if (searchInput && autocompleteList) {
        searchInput.addEventListener('input', function() {
            ...
            debounceTimer = setTimeout(async () => {
                try {
                    const url = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent('臺北市 ' + query)}&countrycodes=tw&limit=6`;
